...
G4VPhysicalVolume* GaletDetectorConstruction::DefineVolumes()
{
  ...
  */
  ...
  //
  /*
  G4double BlockSizeX = 100.*cm;
  G4double BlockSizeY = 100.*cm;
  G4double BlockSizeZ = 100.*cm;
  
  G4VSolid* BlockS 
    = new G4Box("World",           // its name
                 BlockSizeX/2, BlockSizeY/2, BlockSizeZ/2); // its size
  
  G4LogicalVolume* BlockLV
    = new G4LogicalVolume(
                 BlockS,     // its solid˜
                 G4Material::GetMaterial("G4_Al"),  // its material
                 "BlockLV");   // its name

  G4RotationMatrix* Blockrot=new G4RotationMatrix();
  Blockrot->rotateX(0.*degree);
  G4ThreeVector Blocktran;
  Blocktran.set(0,0,60*cm);
  new G4PVPlacement(
                 Blockrot,                // no rotation
                 Blocktran,  // at (0,0,0)
		 BlockLV,
                 "BlockPV",    // its name
                 worldLV,          // its mother  volume
  ...
  BlockLV->SetVisAttributes(BlockVisAtt);
  */
  //
